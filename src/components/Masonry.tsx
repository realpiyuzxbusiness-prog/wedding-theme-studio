...
const Masonry = ({
  ...
  const { gridItems, maxHeight } = useMemo(() => {
    if (!width) return { gridItems: [], maxHeight: 0 };
    ...
    return { gridItems: itemsGrid, maxHeight: Math.max(...colHeights) };
  }, [columns, items, width]);
  ...
  return (
    <div ref={containerRef} className="list masonry-list" style={{ height: maxHeight }}>
      {gridItems.map(item => {
        return (
          <div
            key={item.id}
            ...
            style={{
              cursor: (item.url || onImageClick) ? 'pointer' : 'default'
            }}
          >
