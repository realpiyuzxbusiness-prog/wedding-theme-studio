...
serve(async (req: Request) => {
  try {
    ...
    // Supabase Webhooks send the data in 'record'
    const record = payload.record || payload

    // ── CONFIG ────────────────────────────────────────────────────
    const RESEND_API_KEY   = Deno.env.get("RESEND_API_KEY") ?? "re_j71Z5LS9_L3P9xpEQvXCnLFwZV8WKaggx"
    const OWNER_EMAIL      = "realpiyuzx.services@gmail.com"   // Your email for free tier
    ...
    const sendEmail = async (to: string, subject: string, html: string) => {
      const res = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          from: "Wedding Theme Studio <onboarding@resend.dev>",
...
<table width="600" style="max-width:600px;width:100%;background:#ffffff;box-shadow:0 4px 32px rgba(180,80,100,0.10);">
  ...
  <tr>
    <td style="padding:0 40px 32px;text-align:center;">
      <a href="https://api.whatsapp.com/send/?phone=${(phone || "").replace(/\D/g,"")}" style="display:inline-block;background:#25D366;color:#fff;padding:12px 25px;text-decoration:none;font-size:12px;font-weight:bold;">💬 WhatsApp Client</a>
    </td>
  </tr>
</table>
</td></tr>
...
<table width="600" style="max-width:600px;width:100%;background:#ffffff;">
  ...
  <tr><td style="padding:40px;text-align:center;">
    ...
    <p style="color:#B08090;">We look forward to capturing your beautiful story.</p>
  </td></tr>
</table>
</td></tr>
</table>
    ...
    // SEND BOTH to owner for free tier compliance (or client if using domain)
    // Since you're on Resend Free Tier, you can ONLY send to your OWN email.
    // So both emails will go to you so you can see them.
    await Promise.all([
      sendEmail(OWNER_EMAIL, `🌸 New Booking: ${name} | ${formatDate(wedding_date)}`, ownerHtml),
      sendEmail(OWNER_EMAIL, `Copy of Client Confirmation for ${name}`, clientHtml)
    ])
